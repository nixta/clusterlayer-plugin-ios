...
{
    self = [self initWithRenderer:originalRenderer];
    if (self) {
...
-(id)init {
    ...
    if (self) {
        self.clusterGenBlock = ^(AGSCluster *cluster) {
            ...
            UIColor *c = cluster.featureCount < 10?smallClusterColor:(cluster.featureCount < 100?mediumClusterColor:largeClusterColor);
            ...
            if (cluster.featureCount > 99) fontSize = fontSize * 0.8;
            AGSTextSymbol *countSymbol = [AGSTextSymbol textSymbolWithText:[NSString stringWithFormat:@"%d", cluster.featureCount]
