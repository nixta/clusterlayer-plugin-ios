...
class ClusterLayer<M: ClusterManager>: AGSFeatureCollectionLayer {
    ...
    init(mapView: AGSMapView, featureLayer: AGSFeatureLayer) {
        ...
        unclusteredItemsTable.displayName = "Unclustered Items Table"
        ...
        // #1 Get the data
        sourceLayer.featureTable?.queryFeatures(with: AGSQueryParameters.all(), completion: { [weak self] (sourceFeatureResults, error) in
        ...
        initializationGroup.notify(queue: .global(qos: .userInitiated)) { [weak self] in
            ...
            self.updateClusterLayerDisplay(for: mapView)
            ...
            self.mapScaleObserver = mapView.observe(\.mapScale, options: [.new], changeHandler: { [weak self] (changedMapView, change) in
                ...
                DispatchQueue.main.debounce(interval: 0.2, context: changedMapView, action: {
                    ...
                    self.updateClusterLayerDisplay(for: changedMapView)
                })
            })
    ...
    func updateClusterLayerDisplay(for mapView: AGSMapView) {
...
//            self.updateClusterDisplay(component: .coverages, mapView: mapView)
    ...
    private func updateClusterDisplay(component: ClusterLayerComponent, mapView: AGSMapView) {
